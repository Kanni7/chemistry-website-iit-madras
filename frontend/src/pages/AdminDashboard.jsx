...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // 🔥 THE AUTO-HEAL FIX: If the token is a ghost, destroy it and force login!
  ...
  const toggleUserStatus = async (userId) => {
    try {
      ...
      if (response.ok) {
        ...
        alert("❌ " + (data.error || data.msg || "Failed to change status."));
      }
    } catch (err) {
  ...
  const handleRegisterSubmit = async (e) => {
    e.preventDefault();
    try {
      // 🔥 THE ENDPOINT FIX: Pointing to auth/register
      ...
      if (response.ok) {
        ...
        fetchUsers(); 
        alert("✅ " + (data.message || "User successfully created!"));
        setFormData({email: '', password: '', role: 'student', first_name: '', last_name: '', roll_number: '', program: 'BS', title: 'Prof.', designation: ''});
      } else {
        alert("❌ " + (data.error || data.msg || data.message || "Registration failed."));
      }
    } catch (err) {
      alert("❌ Failed to create user. Check your connection to the server.");
    }
  };
    ...
    <div className="min-h-screen bg-[#f5f6f8] py-10 px-6 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-[#b45309] hover:bg-[#92400e] text-white px-6 py-2.5 rounded-sm font-bold shadow-sm transition-colors">
            + Register New User
          </button>
        </div>
        ...
        <div className="bg-white p-4 border border-[#e5e7eb] shadow-sm mb-6 flex gap-4 justify-between">
          <div className="relative w-1/2">
            ...
            <input type="text" placeholder="Search by name or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 focus:outline-none focus:border-[#b45309] rounded-sm" />
          </div>
          <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="border border-gray-300 p-2 focus:outline-none focus:border-[#b45309] rounded-sm bg-white">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredUsers.map(user => (
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full ${user.role === 'admin' ? 'bg-purple-100 text-purple-700' : user.role === 'faculty' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{user.role}</span>
                  </td>
